...
def initialize(seed, optim, elbo, data, n_batches):
    ...
    global_guide = AutoDelta(
        poutine.block(model, expose=["beta0", "beta1", "gamma", "pi"]),
...
def fit_Binomial(gRNA, adata_crispr, batch_list, output_dir, seed):
    '''
    Fits Binomial model for UMI counts of one gRNA 
    Args:
    ...
    for step in range(n_steps):
        ...
        loss = svi.step(subsample, len(batch_list))
...
def get_perturbed_cells(adata_crispr, estimates, gRNA):
    ...
    data['prob0'] = sigmoid(np.exp(beta0 + gamma_cells))
    data['prob1'] = sigmoid(np.exp(beta0 + beta1 + gamma_cells))
    
    # get probabilities for the two mixture components
    data['p0'] = binom.pmf(data['gRNA_counts'], data['seq_depth'], data['prob0']) * (1 - pi)
    data['p1'] = binom.pmf(data['gRNA_counts'], data['seq_depth'], data['prob1']) * pi

    # filter for perturbed cells
    perturbed_cells = data[data['p1'] > data['p0']]
...
def main():
    ...
    print('Fit Binomial Model for each gRNA')
    for gRNA in gRNA_list:
        # fit the model to infer the parameters
        loss, map_estimates = fit_Binomial(gRNA, adata_crispr, config['batch_list'], output_dir, config['seed'])
