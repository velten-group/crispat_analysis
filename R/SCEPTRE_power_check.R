...
discovery_pairs <- filter(discovery_pairs, response_id %in% discovery_pairs$grna_target)
...
} else if (nrow(sceptre_object@response_matrix[[1]]) < 100){
  formula_object = formula(~ log(response_n_nonzero) + log(response_n_umis) +
                             log(grna_n_nonzero + 1) + log(grna_n_umis + 1))
} else {
  formula_object = formula(~ log(response_n_nonzero) + log(response_n_umis) +
...
times <- data.frame('step' = c('create SCEPTRE', 'set analysis parameters', 'read in guide assignment',
                      'quality control', 'calibration check', 'power check'), 
           'time' = c(t1-t0, t2-t1, t3-t2, t4-t3, t5-t4, t6-t5))
