...
method_names <- data.frame('method' = c("2-Beta", "3-Beta", "Cellranger", "Replogle", 
                                        "SCEPTRE", "Negative Binomial", "Maximum", "Binomial",
...
get_run_time <- function(file_dir, name, method_names){
  ...
  if (method_name %in% c('SCEPTRE', 'Negative Binomial')){
...
method_names_gc <- data.frame('method' = c("2-Beta", "3-Beta", "Cellranger", "Replogle", 
                                        "SCEPTRE", "Negative Binomial", "Maximum", "Binomial",
                                        "top_5%_cells", "UMI_t30", "ratio_50%"), 
...
ggplot(gc_times, aes(x = reorder(method, h), y = h, fill = method)) + 
  geom_bar(stat = 'identity') + 
  theme_bw() + 
...
ggplot(power_times, aes(x = reorder(method, h), y = mins, fill = method)) + 
  geom_bar(stat = 'identity') + 
  theme_bw() + 
...
ggplot(filter(discovery, step == 'calibration check'), aes(x = reorder(method, h), y = h, fill = method)) + 
  geom_bar(stat = 'identity') + 
  theme_bw() + 
...
ggplot(filter(discovery, step == 'discovery analysis'), aes(x = reorder(method, h), y = h, fill = method)) + 
  geom_bar(stat = 'identity') + 
  theme_bw() + 
